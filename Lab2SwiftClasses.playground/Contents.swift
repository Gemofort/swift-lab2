...
class FoolCardGame: CardGame {
    static var minCardsInHand: Int = 0
    ...
    func nextPlayer(after index: Int) -> CardPlayer? {
        if (self.players.count == 0 || self.players.count == 1) {
            return nil;
        }
        if (index + 1 >= self.players.count) {
    ...
    func play() {
        var currentPlayer: CardPlayer = self.players[0];
        var currentCards: [Card] = [];
        var currentIndex = 0;

        for var player in self.players {
            player.fillHand(from: self.deck, for: 6)
        }

        currentCards.append(currentPlayer.playCard());

        while nextPlayer(after: currentIndex) != nil {
            currentPlayer = self.players[currentIndex];
            var next = nextPlayer(after: currentIndex);
            
            if (next?.hand.count != 0) {
                if (currentCards.count != 0) {
                    let card = next?.cover(for: currentCards[currentCards.count - 1])
                    ...
                    if (card == nil) {
                        ...
                        currentIndex = self.players.firstIndex(where: { $0.name == next?.name } ) ?? 0;
                    }
                    break;
                } else {
                    currentCards.append(currentPlayer.playCard());
                ...
                if let index = self.players.firstIndex(where: { $0.name == next?.name } ) ?? 0 {
...
var newGame: CardGame? = FoolCardGame(with: deck);
print(players)
